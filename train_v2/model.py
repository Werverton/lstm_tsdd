...
class SingleScreenModel(BaseModel):
    ...
    def build_model(self):
        ...
        self.pool5_up = tf.nn.relu(tf.nn.conv2d_transpose(input=self.pool5_heat,
...
class MultipleScreenModel(BaseModel):
    ...
    def build_summary(self):
        # summary
        tf.compat.v1.summary.scalar("heatmap_loss", self.heatmap_loss)
        tf.compat.v1.summary.scalar("interact_loss", self.interact_loss)
        tf.compat.v1.summary.scalar("total_loss", self.total_loss)
        tf.compat.v1.summary.image("input_images",
                         self.input_images,
        ...
        tf.summary.image("sample_heatmaps",
                         ...
                         max_outputs=self.batch_size*self.frame_num)
        """
        tf.compat.v1.summary.image("true_heatmaps",
                         self.true_heats,
                         max_outputs=self.batch_size)
        tf.compat.v1.summary.image("predict_heatmaps",
                         self.predict_heatmaps,
                         max_outputs=self.batch_size)
        tf.compat.v1.summary.histogram("true_interacts", self.true_interacts)
        tf.compat.v1.summary.histogram("predict_interacts", self.predict_interacts)

        tf.compat.v1.summary.histogram("conv1_activation", self.conv1)
        tf.compat.v1.summary.histogram("conv2_activation", self.conv2)
        tf.compat.v1.summary.histogram("conv3_activation", self.conv3)
        tf.compat.v1.summary.histogram("conv4_activation", self.conv4)
        tf.compat.v1.summary.histogram("conv5_activation", self.conv5)
        tf.compat.v1.summary.histogram("conv1_gradient", tf.gradients(ys=self.total_loss, xs=self.conv1))
        tf.compat.v1.summary.histogram("conv2_gradient", tf.gradients(ys=self.total_loss, xs=self.conv2))
        tf.compat.v1.summary.histogram("conv3_gradient", tf.gradients(ys=self.total_loss, xs=self.conv3))
        tf.compat.v1.summary.histogram("conv4_gradient", tf.gradients(ys=self.total_loss, xs=self.conv4))
        tf.compat.v1.summary.histogram("conv5_gradient", tf.gradients(ys=self.total_loss, xs=self.conv5))

        tf.compat.v1.summary.histogram("pool3_heat_out_activation", self.pool3_heat_out)
        tf.compat.v1.summary.histogram("pool4_heat_out_activation", self.pool4_heat_out)
        tf.compat.v1.summary.histogram("pool5_heat_out_activation", self.pool5_heat_out)
        tf.compat.v1.summary.histogram("pool3_heat_out_gradient", tf.gradients(ys=self.total_loss, xs=self.pool3_heat_out))
        tf.compat.v1.summary.histogram("pool4_heat_out_gradient", tf.gradients(ys=self.total_loss, xs=self.pool4_heat_out))
        tf.compat.v1.summary.histogram("pool5_heat_out_gradient", tf.gradients(ys=self.total_loss, xs=self.pool5_heat_out))

        tf.compat.v1.summary.histogram("pool3_up_filters_data", self.pool3_up_filters)
        tf.compat.v1.summary.histogram("pool4_up_filters_data", self.pool4_up_filters)
        tf.compat.v1.summary.histogram("pool5_up_filters_data", self.pool5_up_filters)
        tf.compat.v1.summary.histogram("pool3_up_filters_gradient", tf.gradients(ys=self.total_loss, xs=self.pool3_up_filters))
        tf.compat.v1.summary.histogram("pool4_up_filters_gradient", tf.gradients(ys=self.total_loss, xs=self.pool4_up_filters))
        tf.compat.v1.summary.histogram("pool5_up_filters_gradient", tf.gradients(ys=self.total_loss, xs=self.pool5_up_filters))
